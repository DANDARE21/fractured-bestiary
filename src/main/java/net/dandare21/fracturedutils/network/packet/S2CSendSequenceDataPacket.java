...
import net.dandare21.fracturedutils.client.ClientOrchestratorHandler;
...
public class S2CSendSequenceDataPacket {
    private final Map<String, String> sequenceFiles;

    public S2CSendSequenceDataPacket(Map<String, String> sequenceFiles) {
        this.sequenceFiles = sequenceFiles != null ? sequenceFiles : new HashMap<>();
    }

    public S2CSendSequenceDataPacket(FriendlyByteBuf buf) {
    ...
    public void handle(Supplier<NetworkEvent.Context> ctxSupplier) {
        ...
        ctx.enqueueWork(() -> {
            DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> ClientOrchestratorHandler.openScreen(this.sequenceFiles));
        });
